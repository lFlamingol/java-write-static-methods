...
`git clone https://github.com/ByteLegendQuest/java-write-static-methods`

A project directory named `java-write-static-methods` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-write-static-methods` directory which is created just now.
